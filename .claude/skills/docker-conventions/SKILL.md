...
**Exclure `Dockerfile` dans `.dockerignore` ne casse pas `az acr build`, contrairement à une première hypothèse retenue puis invalidée sur ce projet.** La CLI ré-ajoute toujours le Dockerfile au tar envoyé au service de build distant, indépendamment des règles `.dockerignore` (`_pack_source_code` dans `azure/cli/command_modules/acr/_archive_utils.py`, dépôt `Azure/azure-cli`). Le vrai piège d'`az acr build` porte sur `--file`, voir la convention pipeline correspondante (`azure-pipelines-conventions`) : un `--file` **explicite** se résout relatif au répertoire de travail de la CLI, pas au contexte passé en dernier argument — contrairement au comportement par défaut (`--file` omis) qui, lui, se résout bien relatif au contexte.
...
**Mais vérifier d'abord quelle image chaque consommateur déploie réellement.** Au moment du changement de port (Sprint 1), `terraform/` et `k8s/` pointaient tous deux `rgy.k8s.devops-svc-ag.com/polytech/vote:1.0.1` — l'image préconstruite d'Avisto, pas celle buildée ici. Les « aligner » sur le nouveau port les aurait cassés. Depuis (Sprint 4), `terraform/` tire l'image du vote de l'ACR du projet comme le worker : le couplage existe désormais bien côté Terraform (`WEBSITES_PORT = "8000"` posé sur `azurerm_linux_web_app.vote`). Seul `k8s/` pointe encore l'image d'Avisto — ce dossier reste hors de la piste d'hébergement retenue (App Service, pas AKS).
