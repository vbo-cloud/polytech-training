...
- Pattern de nommage : `{type}-{role?}-{project}-{environment}-{region}-{index?}`
  ...
  - Préfixes : `rg`, `vnet`, `snet`, `pe` (Private Endpoint), `pdnsl` (Private DNS Zone Virtual Network Link), `asp` (App Service Plan), `app` (Web App), `redis`, `acr`, `psql`, `appi` (Application Insights), `id` (User Assigned Identity)
  - Exemples : `rg-poly-dev-frc`, `asp-poly-dev-frc`, `snet-platform-poly-dev-frc`, `pe-redis-poly-dev-frc`
  - **Noms imposés par Azure** : certaines ressources n'ont pas de nom libre et sortent du pattern. `azurerm_private_dns_zone` doit porter exactement le nom de zone privatelink du service concerné — le renommer casse la résolution. Ce nom varie selon le type de service, pas seulement selon le pattern projet : `privatelink.redis.azure.net` pour Azure Managed Redis, `privatelink.redis.cache.windows.net` pour l'ancien Azure Cache for Redis (classique). Vérifier la doc Microsoft du service avant de « corriger » un nom qui ne suit pas le pattern.
...
- **Le state ne se versionne jamais.** `*.tfstate*`, `*.tfplan`, `tfplan*` et `.terraform/` doivent figurer dans `.gitignore` — à vérifier, pas à supposer. Le motif `*.tfplan` ne couvre pas le fichier produit par `terraform plan -out=tfplan`, qui n'a pas de point : les deux formes sont nécessaires. Le state stocke en clair tout ce que les providers remontent — ici `azurerm_managed_redis.redis.default_database[0].primary_access_key`, injectée dans les `app_settings` de la Web App. `.terraform.lock.hcl` fait exception : il se commit, il fige les versions de providers.
- `terraform.tfvars` ne doit jamais être commit s'il contient des secrets. Celui de ce projet ne contient que des valeurs non sensibles (location, tags d'image) : son tracking est un choix assumé, pas un oubli. Dès qu'une valeur sensible y entre, il sort du suivi Git.
...
- `azurerm_managed_redis` — c'est une **file de messages transitoire**, pas un stockage : les votes y passent quelques millisecondes avant que le worker ne les écrive en Postgres. Rien à protéger.
...
- **Toute ressource PaaS dotée d'un private endpoint doit couper l'accès public.** Les providers laissent l'accès public ouvert par défaut : sans ce réglage, le private endpoint est décoratif et la ressource reste joignable depuis Internet avec sa seule clé. Le nom et le type de l'argument varient selon la ressource — vérifier le schéma du type avant d'écrire la ligne, ne pas la recopier d'une ressource à l'autre :
  - `azurerm_managed_redis` : `public_network_access = "Disabled"` (string, pas un bool).
  - `azurerm_redis_cache` (ancien Azure Cache for Redis, classique) : `public_network_access_enabled = false` (bool). Ce nom d'argument ne s'applique **pas** à `azurerm_managed_redis`.
  - Postgres suit le même principe par un mécanisme différent — un serveur flexible ne se met pas derrière un private endpoint, l'accès privé passe par un sous-réseau délégué.
  - L'ACR, lui, reste volontairement public : le SKU Basic ne supporte pas les private endpoints (Premium serait ~4x le coût), justifié en commit.
...
- `azurerm_redis_cache` (ancien Azure Cache for Redis, classique — retiré des nouvelles créations, cf. section nommage) : `minimum_tls_version = "1.2"`. `azurerm_managed_redis` n'expose pas cet argument : TLS 1.2+ est imposé par défaut sur toutes les connexions, rien à configurer ([doc Microsoft](https://learn.microsoft.com/en-us/azure/redis/secure-azure-managed-redis)).
- **`azurerm_managed_redis` n'active que Microsoft Entra ID à la création — poser `default_database { access_keys_authentication_enabled = true }` explicitement si les consommateurs s'authentifient par clé (`primary_access_key`/`secondary_access_key`).** Sans cette ligne, la clé existe et se lit toujours dans le state, mais le serveur refuse de s'en servir pour authentifier : le client échoue au runtime (`AuthenticationError` / `NOAUTH` selon la bibliothèque), invisible à `plan`/`validate` puisque c'est un comportement serveur, pas une erreur de configuration. Vu sur `redis-poly-dev-frc` : découvert seulement au premier test bout-en-bout du flux applicatif en Azure, plan et apply propres jusque-là.
