...
- **Le state ne se versionne jamais.** `*.tfstate*`, `*.tfplan`, `tfplan*` et `.terraform/` doivent figurer dans `.gitignore` — à vérifier, pas à supposer. Le motif `*.tfplan` ne couvre pas le fichier produit par `terraform plan -out=tfplan`, qui n'a pas de point : les deux formes sont nécessaires. Le state stocke en clair tout ce que les providers remontent — ici `azurerm_redis_cache.primary_access_key`, injectée dans les `app_settings` de la Web App. `.terraform.lock.hcl` fait exception : il se commit, il fige les versions de providers.
