...
- Un vrai `terraform plan` de pipeline, déclenché sur cette PR, a échoué sur `azurerm_role_assignment.vote_acr_pull` : « Missing required argument ... `principal_id` ». Donner à `vote` — déjà déployé sans identité — une identité `SystemAssigned` et créer dans le même `apply` le role assignment qui dépend de son `principal_id` est un cas que le provider `azurerm` ne sait pas résoudre au `plan` (bug connu, sans correctif officiel). `worker` et `result` n'étaient pas concernés : les deux sont créés avec leur identité et leur rôle dans le même `apply` initial, scénario que ce bug n'affecte pas — c'est pour ça que la relecture `reviewer` d'origine (commit `e17f95e`) n'avait rien détecté.
- Correctif : les trois identités `SystemAssigned` par app et leurs trois `azurerm_role_assignment` séparés (`worker_acr_pull`, `vote_acr_pull`, `result_acr_pull`) remplacés par une seule identité `azurerm_user_assigned_identity.acr_pull` (`id-acrpull-${local.base_name}` — préfixe `id` ajouté à `terraform-conventions/SKILL.md`) et un seul `azurerm_role_assignment.acr_pull`.
- Un second passage `reviewer`, dédié à ce commit d'identité partagée, a trouvé un bug bloquant avant même un nouveau run de pipeline : `container_registry_use_managed_identity = true` seul ne suffit pas avec une identité `UserAssigned` — il faut aussi préciser `container_registry_managed_identity_client_id`, sinon App Service retombe sur une identité système qui n'existe plus sur aucune des trois apps, cassant le pull d'image au runtime sans que `plan`/`validate` ne le voient. Ajouté sur les trois `site_config`.
- Risque de transition documenté dans `SUIVI.md`, pas passé sous silence : `worker` est la seule des trois apps déjà déployée en prod avec une identité système fonctionnelle, et ce commit la bascule vers l'identité partagée dans le même `apply` qui détruit son ancien role assignment. Vérifié que le bloc `identity` ne force pas de remplacement de la ressource (mise à jour en place), sans garantie de propagation Entra ID du nouveau rôle avant la destruction de l'ancien — fenêtre de quelques minutes, confirmé que rien d'autre dans ce dépôt ne dépend de l'identité système du worker.
- Nouveau run de pipeline sur la PR, une fois le correctif d'identité en place : le job `PublishResultImage` (premier `az acr build` réel sur `result/` — ce service restait avant sur le registre d'Avisto) a échoué à `COPY package*.json .` avec « When using COPY with more than one source file, the destination must be a directory and end with a / ». Le glob matche `package.json` **et** `package-lock.json` vers une seule destination `.` ; BuildKit (Docker Desktop, utilisé par `docker compose build` en local depuis le Sprint 1) l'accepte sans broncher, le builder classique qu'utilise `az acr build` non. Corrigé en `COPY package*.json ./`, documenté dans `docker-conventions/SKILL.md` et dans `SUIVI.md` (Sprint 4).
...
- **Identité `AcrPull` partagée (`UserAssigned`) retenue plutôt qu'un bootstrap manuel de l'identité de `vote` ou une scission du rôle en deux merges — les deux autres options proposées.** Vincent a posé la question directe (« il y a déjà une identité qui a acr pull non ? »), ce qui a mené à corriger que non — chaque app avait sa propre identité système, non réutilisable — puis à retenir l'option qui corrige la cause plutôt que de la contourner : une identité `UserAssigned` est une ressource à part entière, son `principal_id` est un attribut de premier niveau connu dès sa propre création, indépendamment des ressources qui la référencent ensuite. Le problème d'ordonnancement disparaît structurellement, et un seul rôle `AcrPull` reste à maintenir au lieu de trois.
- **Le bug d'ordonnancement du `principal_id` n'était détectable par aucune relecture de code, seulement par un vrai run de `terraform plan`.** Le commit d'origine (`e17f95e`, entrée #16 initiale) suivait le même schéma « identité `SystemAssigned` + role assignment » déjà utilisé sans problème pour `worker` et `result` en Sprint 4 ; rien dans le diff ne distinguait le cas de `vote` (ressource déjà déployée) de ceux-là (ressources neuves). Seul le `plan` réel en CI sur cette PR a révélé la limitation du provider `azurerm`, confirmée ensuite par plusieurs issues ouvertes et non corrigées du dépôt `hashicorp/terraform-provider-azurerm` (dont #26731 et #7009).
- **Même famille de leçon que les entrées #14/#15 — un Dockerfile qui marche en local peut casser uniquement en CI, à cause d'une différence de comportement entre le builder local et celui d'`az acr build`, invisible à toute relecture de code ou `terraform plan`/`validate`.** Différence avec #14/#15 : la cause exacte était ici directement lisible dans le message d'erreur du log de build (fourni en entier par Vincent), pas besoin de redescendre dans le code source d'`azure-cli` pour la confirmer.

---

## #17 — feature/redis-access-keys-auth

**Contexte avant** : `vote` et `result` venaient tout juste d'être déployés sur Azure pour la première fois (entrée #16, PR #10 puis #11, mergées). C'était donc le premier test réel du flux complet `vote → worker → result` dans le navigateur, pas seulement des `plan`/`apply` propres — jusque-là, `vote` tournait sur l'image préconstruite d'Avisto et le trajet réel d'un vote via Redis n'avait jamais été exercé en Azure.

**Objectif** : faire fonctionner un vote de bout en bout en environnement Azure, en corrigeant la cause réelle du blocage observé.

**Ce qui a été fait** :

- Diagnostic : voter restait bloqué ~20 secondes puis renvoyait une 500. Confirmé en direct via `az webapp log tail` (activé par `az webapp log config --docker-container-logging filesystem`) sur le conteneur `vote` : `redis.exceptions.AuthenticationError: HELLO must be called with the client already authenticated...`. Confirmé côté ressource Azure avec `az resource show` sur la base Redis Enterprise (`Microsoft.Cache/redisEnterprise/databases`) : `accessKeysAuthentication: "Disabled"`.
- Cause : Azure Managed Redis n'active que Microsoft Entra ID à la création (« secure from the start », documenté par Microsoft). `primary_access_key` existe déjà et est injectée dans les deux connection strings (`vote` via `redis.from_url()`, `worker` via StackExchange.Redis), mais le serveur refuse de s'en servir pour authentifier tant que ce réglage n'est pas explicite. Ni `plan` ni `validate` ne pouvaient le voir — c'est une politique serveur, pas une erreur de configuration.
- `terraform/main.tf` : le bloc `default_database {}` (vide jusque-là) de `azurerm_managed_redis.redis` devient `default_database { access_keys_authentication_enabled = true }`.
- Documenté dans `SUIVI.md` (Sprint 4, à la suite de la section Redis existante) et dans `.claude/skills/terraform-conventions/SKILL.md` (nouvelle puce dans la checklist sécurité), pour que la prochaine ressource Managed Redis du projet ne reproduise pas l'oubli.
- Passe `reviewer` sur ce premier commit : aucun point bloquant, mais deux vérifications faites contre l'état réel plutôt que sur la foi du commentaire — `access_keys_authentication_enabled` confirmé comme nom exact de l'argument via `terraform providers schema -json`, et un `terraform plan` réel contre l'état déployé confirmé en mise à jour en place (`will be updated in-place`, 0 à détruire), pas une recréation du Redis. Seul retour : le commentaire HCL au-dessus du bloc (« obligatoire à la création, même vide ») devenu trompeur puisque le bloc n'est plus vide — reformulé en « quel que soit son contenu » dans un second commit.

**Décisions techniques** :

- **Ce n'est pas un bug introduit par la branche précédente.** Le flux vote→worker n'avait simplement jamais été testé bout-en-bout en Azure avant cette branche : `vote` pointait sur l'image préconstruite d'Avisto jusqu'à hier (entrée #16).
