...
- [x] **`vote` et `result` basculés sur l'ACR du projet**, même modèle que le worker (image ACR, identité managée, `ignore_changes` sur le tag). Le registre externe d'Avisto ne coexiste plus qu'avec `k8s/`, resté hors de la piste d'hébergement retenue (App Service, décision #7). `vote_registry_url` est supprimée : les trois variables d'image (`web_app_{worker,vote,result}_docker_image_name`) suivent maintenant le même schéma `repository:tag` dans l'ACR.
  ...
  - **Identité `AcrPull` repensée en cours de route : une identité `UserAssigned` partagée (`azurerm_user_assigned_identity.acr_pull`) remplace les trois identités `SystemAssigned` par app initialement prévues.** Cause : donner à `vote` — une ressource déjà déployée sans identité — une identité `SystemAssigned` et créer dans le même `apply` le `azurerm_role_assignment` qui en dépend fait échouer le `terraform plan` (« Missing required argument: principal_id »). Bug connu du provider `azurerm`, sans correctif officiel : le `principal_id` d'une identité `SystemAssigned` n'existe qu'après la création de la ressource qui la porte, et Terraform ne peut pas le résoudre pour un `apply` qui ferait les deux à la fois — fonctionne pour une ressource neuve (`worker` et `result`, créés avec leur identité et leur rôle d'un coup) mais pas pour une ressource déjà en place (`vote`). Une identité `UserAssigned` est une ressource à part entière : son `principal_id` est connu dès sa propre création, indépendamment des web apps qui l'utilisent ensuite — le problème disparaît, et un seul rôle `AcrPull` suffit pour les trois apps au lieu de trois.
    - `container_registry_managed_identity_client_id` posé explicitement sur les trois `site_config` : avec une identité `UserAssigned`, `container_registry_use_managed_identity = true` seul ne suffit pas à dire *laquelle* utiliser — sans lui, App Service retomberait sur une identité système que ces apps n'ont plus, et le pull d'image échouerait au runtime sans que `plan`/`validate` ne le voient.
    - **`worker` est le seul des trois où ce changement porte un risque réel : il tourne déjà en prod avec une identité système fonctionnelle.** Ce commit la fait basculer vers l'identité partagée dans le même `apply` qui détruit l'ancien `azurerm_role_assignment.worker_acr_pull` — une mise à jour en place côté provider (le bloc `identity` ne force pas de remplacement de la ressource), mais sans garantie que le nouveau rôle `AcrPull` ait fini de se propager dans Entra ID avant que l'ancien ne disparaisse. Fenêtre de quelques minutes à surveiller sur le prochain déploiement du worker si un pull d'image échoue juste après ce merge — se recale de lui-même, pas besoin d'intervention si ça arrive. Vérifié : dans ce dépôt, l'identité système du worker n'était utilisée que pour `AcrPull` (aucune autre référence dans `terraform/`, ni de rôle accordé à la main hors Terraform documenté dans ce fichier) — rien d'autre ne dépend d'elle.
  ...
  - **`result/Dockerfile` cassait uniquement en CI, jamais en local.** `COPY package*.json .` matche deux fichiers (`package.json`, `package-lock.json`) vers une seule destination. BuildKit (Docker Desktop, `docker compose build` local) l'accepte ; le builder classique qu'utilise `az acr build` le refuse : `When using COPY with more than one source file, the destination must be a directory and end with a /`. `result/` n'étant construit par ce pipeline que depuis cette branche (avant, il restait sur le registre d'Avisto), c'est le premier `az acr build` réel qui l'a révélé. Corrigé (`COPY package*.json ./`), documenté dans `docker-conventions/SKILL.md`.
...
- [x] **Authentification par clé d'accès activée sur Redis (`access_keys_authentication_enabled = true`).** Découvert au premier test réel du flux `vote → worker → result` en Azure (Sprint 4, une fois `vote` et `result` déployés) : Azure Managed Redis n'active que Microsoft Entra ID par défaut à la création — la clé d'accès (`primary_access_key`, déjà lue et injectée dans les deux connection strings) existe mais le serveur refuse de s'en servir pour authentifier tant que ce réglage n'est pas explicite. `vote` (`redis.from_url()`) échouait avec `AuthenticationError: HELLO must be called with the client already authenticated` ; `worker` (StackExchange.Redis) échouait de façon symétrique, silencieusement jusqu'ici faute d'avoir jamais été testé bout-en-bout. Aucun `plan`/`validate` ne pouvait le voir : c'est un comportement du serveur Redis au runtime, pas une erreur de configuration Terraform.
